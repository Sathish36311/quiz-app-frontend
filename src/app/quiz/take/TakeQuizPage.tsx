...
export default function TakeQuizPage() {
  ...
  function computeScore() {
    ...
    for (const question of questions) {
      if (selected[question.id] && selected[question.id] === question.correct_option) {
        quizScore++;
      }
    }
    return quizScore;
      ...
      {isSubmitted && isReview ? (
        <>
                        ...
                        return (
                          <div
                            ...
                            className={`${styles.optionItem} ${
                              isCorrect ? styles.correct : isUser ? styles.userChoice : ""
                            }`}
                          >
          ...
          <div className={styles.reviewPageButtons}>
            <button className={styles.takeQuizBtn} onClick={resetStates}>
              Retake Quiz
            </button>
            <button className={styles.takeQuizBtn} onClick={() => router.push("/home")}>
              Go Home
            </button>
          </div>
        </>
        ...
        <>
          ...
          <div className={styles.buttonGroup}>
            <button className={styles.takeQuizBtn} onClick={() => setIsReview(true)}>
              Review Answers
            </button>
            <button className={styles.takeQuizBtn} onClick={resetStates}>
              Retake Quiz
        ...
        <>
          ...
          <div className={styles.buttonGroup}>
            <button className={styles.takeQuizBtn} disabled={currentIndex === 0} onClick={handlePrev}>
              Previous
            </button>
            {!isLast && (
              <button className={styles.takeQuizBtn} onClick={handleNext}>
                Next
              </button>
            )}
            {isLast && (
              <button className={styles.takeQuizBtn} onClick={onSubmit} disabled={!allAnswered}>
                Submit
              </button>
            )}
          </div>
